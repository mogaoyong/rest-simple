      ...
      <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
        ...
        <el-menu :default-active="$route.path" :default-openeds="['0']" class="el-menu-dark" @open="handleopen" @close="handleclose"
          ...
          <template v-for="(menu, index) in menus">
            <el-submenu :index="index+''" v-if="menu.isParent">
              <template slot="title"><i :class="menu.icon"></i>{{menu.name}}</template>
              <el-menu-item v-for="child in menu.children" :index="child.path" :key="child.path">
                {{child.name}}
              </el-menu-item>
            </el-submenu>
          </template>
          ...
          <li v-for="(menu,index) in menus" class="el-submenu item">
            <template v-if="menu.isParent">
              <div class="el-submenu__title" style="padding-left: 20px;" @mouseover="showMenu(index,true)"
                   @mouseout="showMenu(index,false)"><i :class="menu.icon"></i></div>
              <ul class="el-menu submenu" :class="'submenu-hook-'+index" @mouseover="showMenu(index,true)"
                  @mouseout="showMenu(index,false)">
                <li v-for="child in menu.children" :key="child.path" class="el-menu-item"
              ...
              <ul>
                <li class="el-submenu">
                  <div class="el-submenu__title el-menu-item"
                       ...
                       :class="$route.path==menu.children[0].path?'is-active':''"
                       @click="$router.push(menu.children[0].path)"><i :class="menu.icon"></i>
                  </div>
                </li>
              </ul>
            </template>
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      let data = await this.$api.post(params, 'resource.menu');
      if (data) {
        _this.menus = data;
      }
      /* this.$api.post(params, 'resource.menu').then(response => {
